...
def actualite_exists(conn, titre, url, libelle, image):
    # Création d'un curseur pour exécuter des requêtes SQL
    cursor = conn.cursor()
    
    # Requête pour vérifier si l'actualité existe déjà dans la base de données
    query = "SELECT COUNT(*) FROM actualitées WHERE titre = %s AND url_actu = %s AND libelle = %s AND url_image = %s"
    data = (titre, url, libelle, image)
    cursor.execute(query, data)
    count = cursor.fetchone()[0]
    
    # Retourne True si l'actualité existe déjà, False sinon
    return count > 0

def insert_actualite(conn, titre, libelle, url, image, date_publication, auteur):
    titre = titre.replace('"', "'")
    ...
    query = "INSERT INTO actualitées (titre, libelle, url_image, url_actu, auteur, date_publication) VALUES (%s, %s, %s, %s, %s, %s)"
    data = (titre, libelle, image, url, auteur, date_publication)
    try:
        cursor.execute(query, data)
        conn.commit()
        print("Actualitée inséré avec succès dans la base de données")
    except mysql.connector.Error as e:
        print("Erreur lors de l'insertion de l'actualitée':", e)

def get_popular_actualites(url, conn):
    ...
    if response.status_code == 200:
        ...
        # Trouve tous les éléments contenant les titres, les libellés, les URL des actualités et les URL des images
        actualite_items = soup.find_all('div', class_='listingResult')[:10]  # Limiter à 10 actualités
        
        # Pour chaque actualité, extrait et insère les informations dans la base de données
        for item in actualite_items:
            ...
            if titre and libelle and image and url and date_publication and auteur:
                # Vérifie si l'actualité existe déjà dans la base de données
                if not actualite_exists(conn, titre, url, libelle, image):
                    # L'actualité n'existe pas encore, donc on peut l'insérer
                    insert_actualite(conn, titre, libelle, url, image, date_publication, auteur)
                else:
                    print("Actualité déjà présent dans la base de données, ignoré")
            else:
                print("Actualité ignoré car des données essentielles sont manquantes")
    else:
        print("Erreur lors de la récupération de la page:", response.status_code)

# URL du site web contenant les actualités populaires
url = "https://global.techradar.com/fr-fr/pro/news"

# Connexion à la base de données
connection = create_connection()

# Appel de la fonction pour récupérer les actualités populaires et les insérer dans la base de données
if connection:
    get_popular_actualites(url, connection)
    connection.close()
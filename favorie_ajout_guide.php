...
if (isset($_SESSION['autoriser']) && $_SESSION['autoriser'] == "oui" && isset($_SESSION['identifiant'])) {
    ...
    try {
        ...
        if ($result_id_utilisateur) {
            ...
            if (isset($_GET['action']) && $_GET['action'] == 'add_favorite' && isset($_GET['guideId'])) {
                // Récupérer l'ID du guide à ajouter aux favoris
                $guideId = $_GET['guideId'];

                $guideId = intval($guideId);

                // Insérer les informations dans la table de favoris
                $sql_insert_favorite = $pdo->prepare("INSERT INTO favories_guides (IdU, IdG) VALUES (:id_utilisateur, :id_guide)");
                $sql_insert_favorite->bindParam(':id_utilisateur', $id_utilisateur, PDO::PARAM_INT);
                $sql_insert_favorite->bindParam(':id_guide', $guideId, PDO::PARAM_INT);
